...
package com.sable.database.connections;

import com.sable.Sable;
import com.sable.contracts.database.StatementInterface;
import com.sable.contracts.database.connections.HostnameDatabase;
import com.sable.database.DatabaseManager;
import com.sable.database.grammar.mysql.*;
import com.sable.database.query.QueryBuilder;
import com.sable.database.schema.Blueprint;
...
public class MySQL extends HostnameDatabase {
    ...
    protected boolean initialize() {
        try {
            ...
            Sable.getLogger().error("MySQL DataSource class missing.", ex);
        }
    ...
    public boolean open() throws SQLException {
        try {
            ...
            Sable.getLogger().error(reason, ex);
    ...
    protected void queryValidation(StatementInterface statement) throws SQLException {
        ...
        switch ((MySQLStatement) statement) {
            case USE:
                ...
                Sable.getLogger().error("Please create a new connection to use a different database.", exception);
                throw exception;
            ...
            case DEALLOCATE:
                ...
                Sable.getLogger().error("Please use the prepare() method to prepare a query.", exception);
                throw exception;
        }
    }
    ...
    public StatementInterface getStatement(String query) throws SQLException {
        ...
        try {
            ...
            Sable.getLogger().error(String.format("Unknown statement: \"%s\"", statement[0]), ex);
        }
    ...
    public boolean hasTable(String table) {
        try {
            ...
            Sable.getLogger().error(String.format("Failed to check if table exists \"%s\": %s", table, ex.getMessage()), ex);
        }
    ...
    public boolean truncate(String table) {
        try {
            ...
            Sable.getLogger().error(String.format("Failed to truncate \"%s\": %s", table, ex.getMessage()), ex);
        }
