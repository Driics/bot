...
package com.sable.cache.adapters;

import com.sable.Sable;
import com.sable.Constants;
import com.sable.cache.CacheItem;
import com.sable.contracts.cache.CacheAdapter;
...
public class FileAdapter extends CacheAdapter {
    ...
    public Object get(String token) {
        ...
        try {
            CacheItem item = Sable.gson.fromJson(new String(Files.readAllBytes(cacheFile.toPath())), CacheItem.class);
            if (item == null) {
                return false;
            }
    ...
    public CacheItem getRaw(String token) {
        ...
        try {
            return Sable.gson.fromJson(new String(Files.readAllBytes(cacheFile.toPath())), CacheItem.class);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    private boolean writeTo(File file, Object value, long seconds) {
        ...
        try {
            ...
            bw.write(Sable.gson.toJson(cacheItem) + "\n");
        } catch (IOException e) {
            e.printStackTrace();
            return false;
        } finally {
