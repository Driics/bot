...
package com.sable.contracts.scheduler;

import com.sable.Sable;
import com.sable.contracts.reflection.Reflectional;
...
public abstract class Job extends TimerTask implements Reflectional {
    ...
    /**
     * Instantiates the job instance with the given Sable application instance, with
     * a delay of 0, period of 1, and time unit of {@link TimeUnit#MINUTES}.
     *
     * @param avaire The Sable application instance.
     */
    public Job(Sable avaire) {
        this(avaire, 0);
    }

    /**
     * Instantiates the job instance with the given Sable application instance and delay,
     ...
     */
    public Job(Sable avaire, long delay) {
        this(avaire, delay, 1);
    }

    /**
     * Instantiates the job instance with the given Sable application instance, delay,
     ...
     */
    public Job(Sable avaire, long delay, long period) {
        this(avaire, delay, period, TimeUnit.MINUTES);
    }

    /**
     * Instantiates the job instance with the given Sable application
     ...
     */
    public Job(Sable avaire, long delay, long period, TimeUnit unit) {
        this.avaire = avaire;
    ...
    protected void handleTask(Task... tasks) {
        for (Task task : tasks) {
            try {
                log.trace("Invoking {}#handle(sable)", task.getClass().getName());
                task.handle(avaire);
            } catch (Exception ex) {
