...
import com.sable.admin.BotAdmin;
import com.sable.ai.IntelligenceManager;
import com.sable.ai.dialogflow.DialogFlowService;
...
import com.sable.blacklist.Blacklist;
import com.sable.cache.CacheManager;
import com.sable.cache.CacheType;
import com.sable.chat.ConsoleColor;
import com.sable.commands.Category;
import com.sable.commands.CategoryDataContext;
import com.sable.commands.CategoryHandler;
import com.sable.commands.CommandHandler;
import com.sable.commands.administration.ChangePrefixCommand;
import com.sable.commands.utility.SourceCommand;
import com.sable.commands.utility.StatsCommand;
import com.sable.commands.utility.UptimeCommand;
import com.sable.config.*;
...
import com.sable.database.serializer.PlaylistSongSerializer;
import com.sable.database.transformers.PlaylistTransformer;
import com.sable.exceptions.InvalidApplicationEnvironmentException;
import com.sable.exceptions.InvalidPluginException;
import com.sable.exceptions.InvalidPluginsPathException;
import com.sable.handlers.EventEmitter;
import com.sable.handlers.MainEventHandler;
import com.sable.handlers.PluginEventHandler;
import com.sable.handlers.events.ApplicationShutdownEvent;
import com.sable.imagegen.RankBackgroundHandler;
import com.sable.language.I18n;
import com.sable.level.LevelManager;
import com.sable.metrics.Metrics;
import com.sable.middleware.*;
import com.sable.middleware.global.IsCategoryEnabled;
import com.sable.mute.MuteManager;
import com.sable.plugin.PluginLoader;
import com.sable.plugin.PluginManager;
import com.sable.scheduler.ScheduleHandler;
import com.sable.servlet.WebServlet;
import com.sable.servlet.routes.*;
import com.sable.shard.ShardEntityCounter;
import com.avairebot.shared.DiscordConstants;
import com.avairebot.shared.ExitCodes;
import com.avairebot.shared.SentryConstants;
import com.sable.time.Carbon;
import com.sable.utilities.AutoloaderUtil;
import com.sable.vote.VoteManager;
...
public class Sable {
    ...
    private static final Logger log = LoggerFactory.getLogger(Sable.class);

    protected static Sable avaire;
    ...
    public Sable(Settings settings) throws IOException, SQLException, InvalidApplicationEnvironmentException {
        ...
        log.info("Bootstrapping Sable v" + AppInfo.getAppInfo().version);
        ...
        AutoloaderUtil.load(Constants.PACKAGE_MIGRATION_PATH, migration -> database.getMigrations().register((Migration) migration), false);

        log.info("Registering database table seeders");
        AutoloaderUtil.load(Constants.PACKAGE_SEEDER_PATH, seeder -> database.getSeeder().register((Seeder) seeder), true);
        ...
        if (getConfig().getString("system-prefix", DiscordConstants.DEFAULT_SYSTEM_PREFIX).equals(defaultPrefix)) {
            ...
            log.error("The system and default prefix can not be the same, stopping the sable");
            System.exit(ExitCodes.EXIT_CODE_NORMAL);
        }
        ...
        if (settings.isGenerateJsonFileMode()) {
            ...
            try (FileWriter file = new FileWriter("commandMap.json")) {
                file.write(Sable.gson.toJson(map));
        ...
        log.info("Creating sable instance and connecting to Discord network");
    ...
    public static Sable getInstance() {
        return avaire;
    }
    ...
    public void shutdown(int exitCode) {
        ...
        getLogger().info("Shutting down sable instance gracefully with exit code " + exitCode);
        ...
        // can resume the music once the sable boots back up.
        ...
        try {
            if (shutdownDelay > 5000L) {
                ...
                // fixed 5 seconds delay, this should help prevent the sable staying up
