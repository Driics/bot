...
public class MusicActivityTask implements Task {
    ...
    private void handleInternalLavaplayer(Sable avaire) {
        for (JDA shard : avaire.getShardManager().getShards()) {
            ...
            try {
                ...
                Sable.getLogger().error("An exception occurred during music activity job: " + e.getMessage(), e);
            }
        }
    }

    private void handleLavalinkNodes(Sable avaire) {
        for (JdaLink link : LavalinkManager.LavalinkManagerHolder.lavalink.getLavalink().getLinks()) {
            ...
            try {
                ...
                Sable.getLogger().error("An exception occurred during music activity job for ID: {} - Message: " + e.getMessage(), guildId, e);
            }
        }
    }

    private void handleEmptyMusic(Sable avaire, @Nullable AudioManager manager, @Nullable JdaLink link, @Nullable GuildMusicManager guildMusicManager, long guildId) {
    ...
    private void handlePausedMusic(Sable avaire, @Nullable AudioManager manager, @Nullable JdaLink link, @Nullable GuildMusicManager guildMusicManager, long guildId) {
    ...
    private int getValue(Sable avaire, String path, int def) {
