...
public class DrainVoteQueueTask implements Task {
    ...
    public void handle(Sable avaire) {
        ...
        if (avaire.getConfig().getBoolean("vote-lock.sync-with-public-sable", false)) {
    ...
    private void acceptViaPublicSync(Sable avaire, Response response, VoteEntity entity) {
    ...
    private void acceptViaDBL(Sable avaire, Response response, VoteEntity entity) {
        if (response.getResponse().code() != 200) {
            return;
        }
    ...
    private void handleRegisteringVote(Sable avaire, User user, Carbon expiresIn, VoteEntity entity) {
