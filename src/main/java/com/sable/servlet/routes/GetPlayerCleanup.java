...
public class GetPlayerCleanup extends SparkRoute {
    ...
    public Object handle(Request request, Response response) throws Exception {
        ...
        for (DataRow dataRow : Sable.getInstance().getDatabase().newQueryBuilder(Constants.PLAYER_EXPERIENCE_TABLE_NAME)
            ...
            if (guild == null || !guild.getId().equalsIgnoreCase(dataRow.getString("guild_id"))) {
                try {
                    guild = Sable.getInstance().getShardManager().getGuildById(dataRow.getString("guild_id"));
