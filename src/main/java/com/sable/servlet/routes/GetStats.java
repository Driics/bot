...
public class GetStats extends SparkRoute {
    ...
    private JSONArray buildShards() {
        ...
        for (JDA shard : Sable.getInstance().getShardManager().getShards()) {
    ...
    private JSONObject buildGlobal() {
        ...
        global.put("guilds", Sable.getInstance().getShardEntityCounter().getGuilds());
        global.put("users", Sable.getInstance().getShardEntityCounter().getUsers());

        JSONObject channels = new JSONObject();
        channels.put("total", Sable.getInstance().getShardEntityCounter().getChannels());
        channels.put("text", Sable.getInstance().getShardEntityCounter().getTextChannels());
        channels.put("voice", Sable.getInstance().getShardEntityCounter().getVoiceChannels());
