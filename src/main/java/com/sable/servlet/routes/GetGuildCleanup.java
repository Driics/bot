...
public class GetGuildCleanup extends SparkRoute {
    ...
    public Object handle(Request request, Response response) throws Exception {
        ...
        for (DataRow row : Sable.getInstance().getDatabase().newQueryBuilder(Constants.GUILD_TABLE_NAME).select("id").get()) {
            if (Sable.getInstance().getShardManager().getGuildById(row.getString("id")) == null) {
