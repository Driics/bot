...
import com.sable.database.transformers.GuildTypeTransformer;
...
public class AddReactionRoleCommand extends Command {
    ...
    public AddReactionRoleCommand(Sable avaire) {
        super(avaire, false);
    }
    ...
    public List<String> getMiddleware() {
        return Arrays.asList(
            ...
            "require:sable,general.manage_roles",
            "throttle:guild,1,5"
        );
    }
        ...
        context.getChannel().getHistory().retrievePast(2).queue(messages -> {
            ...
            try {
                avaire.getDatabase().newQueryBuilder(Constants.REACTION_ROLES_TABLE_NAME)
                    ...
                    .update(statement -> {
                        statement.set("roles", Sable.gson.toJson(reactionTransformer.getRoles()));
                    });
