...
public class MuteRoleCommand extends Command {
    ...
    public MuteRoleCommand(Sable avaire) {
        super(avaire, false);
    }
    ...
    public String getDescription() {
        return "Can be used to set, create, setup, or reset the mute role used for mute related commands on the server, the mute role is what is assigned to users when they're muted, preventing them from talking or speaking in voice channels.\n\nWhen creating a mute role via the sable, the sable will create a new role called `Muted`, and then add permission overrides to all text and voice channels the sable has access to, preventing the role from talking or speaking in the channels. If an existing role is set, the permissions can also be automatically setup using the `setup-permissions` option.";
    }
    ...
    private boolean setMutedRole(CommandMessage context, GuildTransformer guildTransformer, String[] args) {
        ...
        try {
            ...
            Sable.getLogger().error(e.getMessage(), e);
