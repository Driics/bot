...
public class ChangePrefixCommand extends Command {
    ...
    public ChangePrefixCommand(Sable avaire) {
        super(avaire, false);
    }
    ...
    public String getDescription() {
        return "Sets the prefix that should be used for all commands in a given category, if no prefix is provided the category prefix will be reset back to the default instead, each category in Sable can have a different prefix, or you can choose to change them all at the same time.";
    }
    ...
    private void updateGuildPrefixes(CommandMessage context, GuildTransformer transformer) throws SQLException {
        avaire.getDatabase().newQueryBuilder(Constants.GUILD_TABLE_NAME)
            ...
            .update(statement -> {
                statement.set("prefixes", Sable.gson.toJson(transformer.getPrefixes()), true);
            });
    }
}
