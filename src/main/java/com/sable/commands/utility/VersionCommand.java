...
public class VersionCommand extends Command {
    ...
    public VersionCommand(Sable avaire) {
        super(avaire);
    }
    ...
    public List<String> getExampleUsage() {
        return Collections.singletonList("`:command` - Gets the current version of the sable, and displays any changes compared to the master branch if there is any.");
    }
    ...
    public boolean onCommand(CommandMessage context, String[] args) {
        ...
        if (latestVersion == null) {
            return sendErrorMessage(context, "Failed to fetch the latest version of Sable, try again later.");
        }
    ...
    private SemanticVersion getLatestVersion() {
        Object version = avaire.getCache().getAdapter(CacheType.FILE).remember("github.version", 1800, () -> {
            try {
                ...
                Sable.getLogger().error("Failed to get latest version from github", e);
                return null;
            }
        });
