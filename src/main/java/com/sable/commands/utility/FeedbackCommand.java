...
public class FeedbackCommand extends Command {
    ...
    public FeedbackCommand(Sable avaire) {
        super(avaire);
    }
    ...
    public String getDescription() {
        return "Send feedback about Ava back to the developers and the staff team, any message passed to the command will be sent in the [#feedback](https://avairebot.com/support) channel on the [Sable Central](https://avairebot.com/support) server.";
    }
    ...
    private boolean sendFeedback(CommandMessage context, TextChannel feedbackChannel, String message) {
        ...
        placeholderMessage.queue(newMessage -> {
            ...
            try {
                ...
                Sable.getLogger().error("Failed to store feedback in the database: {}", e.getMessage(), e);
            }
        }, err -> {
            Sable.getLogger().error("Failed to send feedback message: " + err.getMessage(), err);
