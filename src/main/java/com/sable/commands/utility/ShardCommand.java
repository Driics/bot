...
public class ShardCommand extends Command {
    ...
    public ShardCommand(Sable avaire) {
        super(avaire);
    }
    ...
    public String getDescription() {
        // If the shard manager is null, we're still starting up the sable.
        if (avaire.getShardManager() == null) {
            return "If you're seeing this message, contact one of the sable developers.";
        }

        return "Displays the status of all the shards for the sable, including their server count, channel count, user count and latency."
