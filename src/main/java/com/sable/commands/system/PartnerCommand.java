...
public class PartnerCommand extends SystemCommand {
    ...
    public PartnerCommand(Sable avaire) {
        super(avaire);
    }
    ...
    public boolean onCommand(CommandMessage context, String[] args) {
        ...
        if (guild == null) {
            return sendErrorMessage(context, "The sable does not share any server with an ID of `{0}`", args[0]);
        }

        if (guild.getRegion().isVip()) {
            context.makeInfo("The **:server** server is a Discord partner, they will always be partnered with the sable because of that.")
        ...
        try {
            ...
            if (args.length == 1) {
                context.makeInfo("The **:server** server is currently **:status** with the sable.")
                    .set("server", guild.getName())
