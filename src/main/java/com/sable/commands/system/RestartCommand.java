...
public class RestartCommand extends ApplicationShutdownCommand {
    ...
    public RestartCommand(Sable avaire) {
        super(avaire);
    }
    ...
    public String getDescription() {
        return "Schedule a time the sable should be automatically-restarted, the sable will shutdown, then start back up again.\nThis requires [sable/watchdog](https://github.com/avaire/watchdog) to work, without it the sable will just shutdown.";
    }

    @Override
    public List<String> getUsageInstructions() {
        return Arrays.asList(
            "`:command now` - Restarts the sable now.",
            "`:command cancel` - Cancels the restart process.",
            "`:command <time>` - Schedules a time the sable should be restarted."
        );
    }
    ...
    public String scheduleShutdown() {
        return "The sable has been scheduled to be restarted in :fromNow.\n**Date:** :date";
    }
