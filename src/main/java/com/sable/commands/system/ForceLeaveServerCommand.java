...
public class ForceLeaveServerCommand extends SystemCommand {
    ...
    public ForceLeaveServerCommand(Sable avaire) {
        super(avaire);
    }
    ...
    public List<String> getUsageInstructions() {
        return Collections.singletonList(
            "`:command <id>` - Leaves the server with the given ID if the sable is on the server."
        );
    }
    ...
    public boolean onCommand(CommandMessage context, String[] args) {
        if (args.length == 0) {
            return sendErrorMessage(context, "You must include the ID of the server you want the sable to leave.");
        }

        try {
            Guild guild = avaire.getShardManager().getGuildById(args[0]);
            if (guild == null) {
                return sendErrorMessage(context, "The sable is not in any guild with an ID of `{0}`", args[0]);
            }

            String name = guild.getName();
            guild.leave().queue();

            context.makeSuccess("The sable has successfully left the **:name** server with an ID of :id")
                .set("name", name)
                .set("id", args[0])
                .queue();
