...
import com.sable.plugin.translators.PluginHolderTranslator;
import com.sable.plugin.translators.PluginLoaderTranslator;
...
public class PluginManager {
     ...
     */
    public PluginManager(Sable avaire) {
        this.avaire = avaire;
     ...
     *                                     the sable doesn't have read access to the directory.
     * @throws InvalidPluginException      This is thrown if the plugin is not valid in some way.
     */
    public void loadPlugins(Sable avaire) throws InvalidPluginsPathException, InvalidPluginException {
     ...
     */
    public void loadPluginsFromIndex(Sable avaire) {
    ...
    public boolean unloadPlugin(Plugin plugin) {
        ...
        while (iterator.hasNext()) {
            ...
            next.unregisterPlugin(Sable.getInstance());
            iterator.remove();
     ...
     * file on the public sable/plugin GitHub repository.
     *
     * @return A list of official plugins approved behind the team of Ava.
     */
    @SuppressWarnings("unchecked")
    public List<PluginHolder> getOfficialPluginsList() {
        Object plugins = Sable.getInstance().getCache().getAdapter(CacheType.MEMORY).remember("plugins", 10800, () -> {
            try {
