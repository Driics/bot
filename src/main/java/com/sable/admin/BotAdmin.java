...
public class BotAdmin {
    ...
    public BotAdmin(Sable avaire, Set<String> botAdmins) {
        this.avaire = avaire;
        ...
        for (String userId : botAdmins) {
            try {
                ...
                log.warn("{} is an invalid sable ID, the ID have not been added to the sable admin whitelist.",
                    userId
                );
            }
        }
     ...
     * @return The sable admin user for the given user ID.
     */
    @Nonnull
     ...
     * will be used to determine if the sable admin role should be checked for or not.
     *
     * @param userId        The ID of the user that should be returned.
     * @param skipRoleCheck Determines if the sable admin check should be preformed or not.
     * @return The sable admin user for the given user ID.
     */
    @Nonnull
    ...
    /**
     * Gets the user from the sable admins set with
     ...
     * Sable Central server, and if the user has the Bot Admin role.
     *
     * @param userId The user ID that should be checked if they're a role admin.
     * @return <code>True</code> if the user has the sable admin role on the Sable Central
