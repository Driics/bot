...
public enum AdminType {
    ...
    /**
     * Represents a user who is defined as a sable admin through the
     * config, giving them access to all system commands.
     */
    BOT_ADMIN(true, CommandPriority.SYSTEM),

    /**
     * Represents a user who isn't defined as a sable admin through the
     * config, but has the "Bot Administrator" role on the official
     * Sable support server, giving them access to run a select
     * few system commands.
     */
    ROLE_ADMIN(true, CommandPriority.SYSTEM_ROLE),

    /**
     * Represents a user who is not a sable admin.
     */
    USER(false);
